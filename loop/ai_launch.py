...
def _child_launch_bridge_enabled(bridge_mode_override: str | None = None) -> bool:
    raw = str(bridge_mode_override or os.environ.get("LOOP_CHILD_LAUNCH_BRIDGE_MODE") or "").strip().lower()
...
def _should_use_tmux_bridge(
    cmd: Sequence[str],
    *,
    bridge_mode_override: str | None = None,
) -> bool:
    if len(cmd) < 2:
        return False
    if str(cmd[0]) != "codex" or str(cmd[1]) != "exec":
        return False
    if not _child_launch_bridge_enabled(bridge_mode_override):
        return False
...
def start_ai_launch(
    *,
    ...
    if _should_use_tmux_bridge(cmd, bridge_mode_override=bridge_mode_override):
