#!/usr/bin/env python3
"""Validate full repo-global stack restoration from surviving reactor only."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-reactor-full-chain-restore][FAIL] {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loop_system_repo_global_reactor_full_chain_restore_") as td:
        ...
        try:
            ...
            if not _wait_until(
                ...
                return _fail("test setup must be able to terminate the old outer repo-global service pids")
            ...
            if not _process_alive(
                ...
                return _fail("repo reactor must remain alive to drive full-chain restoration")
            ...
            if not _wait_until(
                ...
                return _fail(
                    "surviving repo reactor must restore repo control-plane, which must restore the outer residency guard and reconverge the full repo-global stack"
                )
            ...
            if not control_plane_required_events:
                return _fail("reactor full-chain restore must publish canonical repo_control_plane_required")
            ...
            if not residency_required_events:
                return _fail(
                    "reactor full-chain restore must converge through repo-control-plane-published repo_reactor_residency_required facts"
                )
        finally:
            _cleanup_repo_services(repo_root)

    print("[loop-system-repo-global-reactor-full-chain-restore] OK")
    return 0
