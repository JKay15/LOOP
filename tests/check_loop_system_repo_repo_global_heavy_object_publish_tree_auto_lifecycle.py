#!/usr/bin/env python3
"""Validate repo-global auto lifecycle convergence for publish-tree replacements."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-heavy-object-publish-tree-auto-lifecycle][FAIL] {msg}", file=sys.stderr)
    return 2
...
def _persist_anchor_state(state_root: Path) -> None:
    ...
    kernel_state = KernelState(
        task_id="milestone5-publish-tree-auto-lifecycle",
        root_goal="validate repo-global publish-tree auto lifecycle convergence",
        root_node_id="root-kernel",
    )
    root_node = NodeSpec(
        node_id="root-kernel",
        node_kind="kernel",
        goal_slice="supervise repo-global publish-tree auto lifecycle validation",
        parent_node_id=None,
        generation=0,
        round_id="R0",
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_repo_global_publish_tree_auto_lifecycle_") as repo_root:
        ...
        if str(holder.get("reference_holder_kind") or "") != "workspace_publication_staging_root":
            return _fail("publish-tree auto lifecycle fixture must classify the holder as workspace_publication_staging_root")

        old_store_ref = _write_publish_tree(
            repo_root / "artifacts" / "heavy_objects" / "publish" / "old-auto-lifecycle-tree",
        ...
        if int(control_plane.get("pid") or 0) <= 0:
            return _fail("repo-global publish-tree auto lifecycle requires a live repo control-plane service")

        new_store_ref = _write_publish_tree(
            repo_root / "artifacts" / "heavy_objects" / "publish" / "new-auto-lifecycle-tree",
        ...
        if not _wait_until(_auto_lifecycle_ready, timeout_s=40.0):
            return _fail(
                "repo-global control-plane must auto-converge publish-tree replacement through supersession, "
        ...
        if (
            ...
            return _fail("unchanged publish-tree replacement snapshot must converge through exactly one canonical release plus one lifecycle chain")
        ...
        if not replacement_reference_event_id:
            return _fail("publish-tree auto lifecycle test must locate the canonical replacement reference event")
        if str(release_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("publish-tree auto handoff release must preserve heavy_object_reference_replaced trigger kind")
        if str(release_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("publish-tree auto handoff release must link back to the replacing canonical reference event")
        if str(supersession_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("publish-tree auto supersession must preserve heavy_object_reference_replaced trigger kind")
        if str(supersession_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("publish-tree auto supersession must link back to the replacing canonical reference event")
        if str(gc_payload.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("publish-tree auto GC eligibility must preserve heavy_object_auto_supersession trigger kind")
        if str(gc_payload.get("trigger_ref") or "") != str(supersession_event.get("event_id") or ""):
            return _fail("publish-tree auto GC eligibility must link back to the canonical supersession event")
        if str(reclaimed_payload.get("trigger_kind") or "") != "heavy_object_auto_gc_eligibility":
            return _fail("publish-tree auto reclamation must preserve heavy_object_auto_gc_eligibility trigger kind")
        if str(reclaimed_payload.get("trigger_ref") or "") != str(gc_eligibility_event.get("event_id") or ""):
            return _fail("publish-tree auto reclamation must link back to the canonical GC-eligibility event")
        ...
        if str(released_reference.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("publish-tree auto handoff release trace must preserve trigger_kind")
        if str(released_reference.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("publish-tree auto handoff release trace must preserve trigger_ref")
        ...
        if str(supersession_summary.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("publish-tree auto supersession trace must preserve trigger_kind")
        ...
        if str(eligible_object.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("publish-tree auto GC trace must preserve trigger_kind")
        ...
        if str(reclamation_effect.get("trigger_kind") or "") != "heavy_object_auto_gc_eligibility":
            return _fail("publish-tree auto reclamation trace must preserve trigger_kind")
        ...
        if int(old_reference_inventory.get("active_reference_count") or 0) != 0:
            return _fail("replaced publish-tree object must no longer have active authoritative references after handoff release")
        if int(old_reference_inventory.get("released_reference_count") or 0) != 1:
            return _fail("replaced publish-tree object must expose one released authoritative reference after handoff release")
        ...
        if int(new_reference_inventory.get("active_reference_count") or 0) != 1:
            return _fail("replacement publish-tree identity must retain one active authoritative reference after handoff release")
        if list(new_reference_inventory.get("active_reference_refs") or []) != [str(holder_ref)]:
            return _fail("replacement publish-tree identity must preserve the live holder reference after handoff release")
        ...
        if str(old_view.get("retention_state") or "") != "RECLAIMED":
            return _fail("publish-tree auto lifecycle must drive the replaced object to retention_state=RECLAIMED")
        if old_store_ref.exists():
            return _fail("publish-tree auto lifecycle must reclaim the superseded canonical directory ref")
        new_view = query_heavy_object_retention_view(anchor_root, object_id=new_id)
        if str(new_view.get("retention_state") or "") != "REFERENCED":
            return _fail("replacement publish-tree identity must remain REFERENCED after auto handoff convergence")
        ...
        if len(
            ...
            return _fail("unchanged publish-tree replacement snapshot must not emit duplicate auto handoff release facts")
        if len(
            [
                dict(event)
                for event in _committed_events_by_type(anchor_root, "heavy_object_superseded")
            ...
            return _fail("unchanged publish-tree replacement snapshot must not emit duplicate auto supersession facts")

    print(
        "[loop-system-repo-global-heavy-object-publish-tree-auto-lifecycle][OK] "
        "repo-global control-plane deterministically auto-converges publish-tree replacement through canonical handoff release, supersession, GC eligibility, and reclamation"
    )
    return 0
