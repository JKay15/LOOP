#!/usr/bin/env python3
"""Validate repo-global auto-takeover for evaluator publish-tree roots."""
...
def _fail(msg: str) -> int:
    print(
        f"[loop-system-repo-global-evaluator-publish-tree-auto-takeover][FAIL] {msg}",
        file=sys.stderr,
    )
    return 2
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_repo_global_evaluator_publish_tree_auto_takeover_") as repo_root:
        anchor_root = repo_root / ".loop"
        evaluator_workspace_root = (
            repo_root
            / ".loop"
            / "evaluator-publish-tree-auto-run"
            / "artifacts"
            / "evaluator_runs"
            / "lane_a"
            / ".loop"
        ...
        artifact_ref = _write_publish_tree(
            evaluator_workspace_root / "deliverables" / "primary_artifact",
            label="evaluator-shared-publish-tree",
        )
        live_ref = _write_publish_tree(
        ...
        if int(control_plane.get("pid") or 0) <= 0:
            return _fail("repo-global evaluator publish-tree auto-takeover requires a live repo control-plane service")
        ...
        if not _wait_until(_first_takeover_ready, timeout_s=8.0):
            return _fail(
                "repo-global evaluator publish-tree remediation must converge to canonical registration "
        ...
        if len(registration_events) != 1:
            return _fail("initial unchanged evaluator publish-tree snapshot must converge through exactly one auto-generated registration")
        if str(dict(registration_events[0].get("payload") or {}).get("object_ref") or "") != str(
            ...
            return _fail("initial unchanged evaluator publish-tree snapshot must register the canonical content-addressed tree ref")
        if len(reference_events) != 3:
            return _fail("initial unchanged evaluator publish-tree snapshot must converge through exactly three canonical references")
        ...
        if {
            ...
            return _fail("evaluator publish-tree auto-reference events must preserve artifact, live, and staging refs")
        if {
            ...
            return _fail("evaluator publish-tree auto-reference events must preserve all evaluator holder kinds")
        ...
        if int(inventory.get("active_reference_count") or 0) != 3:
            return _fail("evaluator publish-tree reference inventory must report three active refs after auto-takeover")
        if {
            ...
            return _fail("evaluator publish-tree reference inventory must surface all evaluator holder kinds")
        ...
        if bool(sharing_view.get("shareable")):
            return _fail("fully auto-referenced evaluator publish-tree refs must no longer report shareable=true")
        if not bool(sharing_view.get("converged")):
            return _fail("fully auto-referenced evaluator publish-tree refs must report converged=true")
        if str(sharing_view.get("sharing_state") or "") != "CONVERGED":
            return _fail("fully auto-referenced evaluator publish-tree refs must report sharing_state=CONVERGED")
        if int(sharing_view.get("covered_observed_ref_count") or 0) != 3:
            return _fail("evaluator publish-tree sharing view must report all observed refs as covered")
        if int(sharing_view.get("missing_observed_ref_count") or 0) != 0:
            return _fail("evaluator publish-tree sharing view must report zero missing observed refs after convergence")
        ...
        if int(gap_inventory.get("unmanaged_candidate_count") or 0) != 0:
            return _fail("post-takeover evaluator publish-tree gap inventory must report zero unmanaged candidates")
        ...
        if str(referenced_object.get("reference_kind") or "") != "authority_gap_auto_reference":
            return _fail("evaluator publish-tree auto-takeover trace must preserve authority_gap_auto_reference kind")
        if str(referenced_object.get("trigger_kind") or "") != "heavy_object_authority_gap_remediation":
            return _fail("evaluator publish-tree auto-takeover trace must preserve the remediation trigger kind")
        ...
        if len(
            ...
            return _fail("unchanged evaluator publish-tree gap snapshot must not emit duplicate auto-reference facts")

        next_ref = _write_publish_tree(
            repo_root
            / ".loop"
            / "evaluator-publish-tree-auto-run-next"
            / "artifacts"
            / "evaluator_runs"
            / "lane_b"
            / ".loop"
            ...
            / "deliverables"
            / "primary_artifact",
            label="evaluator-next-publish-tree",
        )
        ...
        if not _wait_until(_second_takeover_ready, timeout_s=8.0):
            return _fail(
                "changed evaluator publish-tree snapshot must converge through a second auto-generated registration and reference"
            )
        ...
        if str(dict(registration_events[-1].get("payload") or {}).get("object_id") or "") != next_object_id:
            return _fail("changed evaluator publish-tree snapshot must converge through registration of the new object id")
        if str(dict(registration_events[-1].get("payload") or {}).get("object_ref") or "") != str(
            ...
            return _fail("changed evaluator publish-tree snapshot must converge through registration of the new canonical tree ref")

    print(
        "[loop-system-repo-global-evaluator-publish-tree-auto-takeover][OK] "
        "repo-global evaluator publish-tree remediation deterministically auto-registers and auto-references artifact, live, and staging roots"
    )
    return 0
