#!/usr/bin/env python3
"""Validate canonical supervision_attached events from trusted sidecar startup."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-canonical-supervision-attached-event][FAIL] {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loop_system_canonical_supervision_attached_event_") as td:
        temp_root = Path(td)
        state_root = temp_root / ".loop"
        workspace_root = temp_root / "workspace"
        launch_result_ref = state_root / "artifacts" / "launches" / "canonical-supervision-attached-001" / "attempt_001" / "ChildLaunchResult.json"
            ...
            json.dumps(
                {
                    ...
                    "node_id": "canonical-supervision-attached-001",
        ...
        try:
            ...
            if exit_code != 0:
                return _fail("sidecar main must succeed during canonical supervision_attached coverage")

            attached_events = [
                event
                for event in iter_committed_events(state_root)
                if str(event.get("event_type") or "") == "supervision_attached"
            ]
            if len(attached_events) != 1:
                return _fail("sidecar startup must commit exactly one canonical supervision_attached event")
            payload = dict(attached_events[0].get("payload") or {})
            ...
            if str(payload.get("launch_event_id") or "") != str(launch_result_ref.resolve()):
                return _fail("supervision_attached must preserve launch identity continuity")
            if str(payload.get("lease_owner_id") or "") != f"sidecar:{runtime_ref.resolve()}":
                return _fail("supervision_attached must preserve sidecar lease owner continuity")
            if int(payload.get("lease_epoch") or 0) != 9:
                return _fail("supervision_attached must preserve the reserved lease epoch")
            if int(payload.get("pid") or 0) != int(sidecar_module.os.getpid()):
                return _fail("supervision_attached must preserve the current sidecar pid")
            if not str(payload.get("process_fingerprint") or "").strip():
                return _fail("supervision_attached must preserve process fingerprint evidence")
            if not str(payload.get("process_started_at_utc") or "").strip():
                return _fail("supervision_attached must preserve process start evidence")
            if str(payload.get("runtime_ref") or "") != str(runtime_ref.resolve()):
                return _fail("supervision_attached must preserve runtime marker evidence")
            ...
            if repeat_exit_code != 0:
                return _fail("conflicting same-epoch restart must remain non-fatal during canonical supervision_attached coverage")
            repeated = [
                event
                for event in iter_committed_events(state_root)
                if str(event.get("event_type") or "") == "supervision_attached"
            ...
            if len(repeated) != 1:
                return _fail("conflicting same-epoch restart must not append a second canonical supervision_attached event")
        finally:
    ...
    print("[loop-system-canonical-supervision-attached-event] OK")
    return 0
