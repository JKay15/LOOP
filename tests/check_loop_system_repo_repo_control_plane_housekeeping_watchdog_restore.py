#!/usr/bin/env python3
"""Validate reactor-mediated repo-control-plane restoration after upper-layer loss."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-control-plane-housekeeping-watchdog-restore][FAIL] {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loop_system_repo_control_plane_housekeeping_watchdog_restore_") as td:
        ...
        try:
            ...
            if not _wait_until(
                lambda: (
                    (supervisor := supervisor_module.live_supervisor_runtime(repo_root=repo_root)) is not None
                ...
                return _fail(
                    "direct repo control-plane bootstrap must still converge host supervisor, housekeeping, and repo reactor"
                )
            supervisor_pid = int(dict(supervisor_module.live_supervisor_runtime(repo_root=repo_root) or {}).get("pid") or 0)
            housekeeping_pid = int(dict(live_housekeeping_reap_controller_runtime(repo_root=repo_root) or {}).get("pid") or 0)
            reactor_pid = int(dict(live_repo_reactor_runtime(repo_root=repo_root) or {}).get("pid") or 0)
            if control_plane_pid <= 0 or supervisor_pid <= 0 or housekeeping_pid <= 0 or reactor_pid <= 0:
                return _fail("setup must expose live repo control-plane, host supervisor, housekeeping, and repo reactor pids")
            ...
            if not _wait_until(
                ...
                return _fail("test setup must be able to terminate the original host supervisor pid")
            ...
            if not _process_alive(
                ...
                return _fail("housekeeping controller must still be alive after host supervisor death")
            if not _process_alive(
                reactor_pid,
                "loop_product.runtime.control_plane --run-repo-reactor",
                repo_root=repo_root,
            ):
                return _fail("repo reactor must still be alive after host supervisor death")
            ...
            if restored is None:
                return _fail("surviving housekeeping controller must restore repo control-plane after both old services die")
            ...
            required_events = [
                ...
                and str(dict(event.get("payload") or {}).get("trigger_kind") or "") == "repo_reactor_watchdog"
            ]
            if not required_events:
                return _fail("surviving repo reactor restore must publish canonical repo_control_plane_required")
        finally:
            _cleanup_repo_services(repo_root)

    print("[loop-system-repo-control-plane-housekeeping-watchdog-restore] OK")
    return 0
