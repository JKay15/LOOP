#!/usr/bin/env python3
"""Validate canonical reference inventory for evaluator-staging runtime-heavy-tree holders."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-evaluator-runtime-heavy-tree-reference-inventory-view][FAIL] {msg}", file=sys.stderr)
    return 2
...
def _persist_anchor_state(state_root: Path) -> None:
    ...
    kernel_state = KernelState(
        task_id="milestone5-evaluator-runtime-heavy-tree-reference-inventory-view",
        root_goal="validate evaluator runtime-heavy-tree reference inventory visibility",
        root_node_id="root-kernel",
    )
    root_node = NodeSpec(
        node_id="root-kernel",
        node_kind="kernel",
        goal_slice="supervise evaluator runtime-heavy-tree reference inventory validation",
        parent_node_id=None,
        generation=0,
        round_id="R0",
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_evaluator_runtime_heavy_tree_reference_inventory_") as repo_root:
        ...
        evaluator_workspace_root = (
            repo_root
            / ".loop"
            / "evaluator-runtime-heavy-reference"
            / "artifacts"
            / "evaluator_runs"
            / "lane_a"
            / ".loop"
        ...
        evaluator_artifact_ref = _write_runtime_heavy_tree(
            evaluator_workspace_root / "deliverables" / "primary_artifact",
            label="evaluator-runtime-heavy-reference",
        )
        evaluator_staging_ref = _write_runtime_heavy_tree(
            evaluator_workspace_root
            / "deliverables"
        ...
        artifact_holder = classify_runtime_heavy_tree_reference_holder(evaluator_artifact_ref, repo_root=repo_root)
        if str(artifact_holder.get("reference_holder_kind") or "") != "evaluator_workspace_artifact_root":
            return _fail("evaluator runtime-heavy-tree artifact holder must classify as evaluator_workspace_artifact_root")
        if str(artifact_holder.get("reference_holder_ref") or "") != str(evaluator_artifact_ref.parent):
            return _fail("evaluator runtime-heavy-tree artifact holder must preserve the evaluator primary_artifact root")

        staging_holder = classify_runtime_heavy_tree_reference_holder(evaluator_staging_ref, repo_root=repo_root)
        if str(staging_holder.get("reference_holder_kind") or "") != "evaluator_workspace_publication_staging_root":
            return _fail("evaluator runtime-heavy-tree staging holder must classify as evaluator_workspace_publication_staging_root")
        if str(staging_holder.get("reference_holder_ref") or "") != str(evaluator_staging_ref.parent.parent):
            return _fail("evaluator runtime-heavy-tree staging holder must preserve the evaluator staging root")
        ...
        submit_heavy_object_registration_request(
            ...
            byte_size=byte_size,
            reason="register evaluator runtime heavy tree before attaching evaluator-staging references",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
            registration_kind="evaluator_runtime_heavy_tree_registration",
        )
        ...
        artifact_envelope = submit_heavy_object_reference_request(
            ...
            reference_kind="evaluator_runtime_heavy_tree_reference",
            reason="attach canonical evaluator runtime-heavy-tree artifact reference",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
            trigger_kind="evaluator_runtime_heavy_tree_reference_test",
        ...
        staging_envelope = submit_heavy_object_reference_request(
            ...
            reference_kind="evaluator_runtime_heavy_tree_reference",
            reason="attach canonical evaluator runtime-heavy-tree staging reference",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
            trigger_kind="evaluator_runtime_heavy_tree_reference_test",
        ...
        if not bool(inventory.get("read_only")):
            return _fail("evaluator runtime-heavy-tree reference inventory must stay read-only")
        if int(inventory.get("active_reference_count") or 0) != 2:
            return _fail("evaluator runtime-heavy-tree reference inventory must report two active references")
        if {
            ...
            return _fail("evaluator runtime-heavy-tree reference inventory must surface both evaluator holder kinds")
        if {
            str(item) for item in list(inventory.get("active_reference_refs") or [])
        } != {str(evaluator_artifact_ref), str(evaluator_staging_ref)}:
            return _fail("evaluator runtime-heavy-tree reference inventory must preserve both evaluator refs")
        if inventory.get("gaps"):
            return _fail(f"complete evaluator runtime-heavy-tree reference inventory must not report gaps, got {inventory['gaps']!r}")
        ...
        if str(artifact_referenced_object.get("reference_holder_kind") or "") != "evaluator_workspace_artifact_root":
            return _fail("evaluator runtime-heavy-tree reference trace must preserve the evaluator artifact holder kind")
        ...
        if str(staging_referenced_object.get("reference_holder_kind") or "") != "evaluator_workspace_publication_staging_root":
            return _fail("evaluator runtime-heavy-tree reference trace must preserve the evaluator staging holder kind")

    print(
        "[loop-system-evaluator-runtime-heavy-tree-reference-inventory-view][OK] "
        "evaluator runtime-heavy-tree reference inventory and trace preserve evaluator artifact plus evaluator staging holder truth"
    )
    return 0
