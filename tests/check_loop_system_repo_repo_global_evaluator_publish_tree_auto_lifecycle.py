#!/usr/bin/env python3
"""Validate repo-global auto lifecycle convergence for evaluator publish-tree replacements."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-evaluator-publish-tree-auto-lifecycle][FAIL] {msg}", file=sys.stderr)
    return 2
...
def _persist_anchor_state(state_root: Path) -> None:
    ...
    kernel_state = KernelState(
        task_id="milestone5-evaluator-publish-tree-auto-lifecycle",
        root_goal="validate repo-global evaluator publish-tree auto lifecycle convergence",
        root_node_id="root-kernel",
    )
    root_node = NodeSpec(
        node_id="root-kernel",
        node_kind="kernel",
        goal_slice="supervise repo-global evaluator publish-tree auto lifecycle validation",
        parent_node_id=None,
        generation=0,
        round_id="R0",
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_repo_global_evaluator_publish_tree_auto_lifecycle_") as repo_root:
        ...
        holder_ref = _write_publish_tree(
            repo_root
            / ".loop"
            / "evaluator-publish-tree-auto-lifecycle"
            / "artifacts"
            / "evaluator_runs"
            / "lane_a"
            / ".loop"
            ...
            / "primary_artifact",
            label="evaluator-publish-tree-live",
        )
        holder = classify_publish_tree_reference_holder(holder_ref, repo_root=repo_root)
        if str(holder.get("reference_holder_kind") or "") != "evaluator_workspace_publication_staging_root":
            return _fail("evaluator publish-tree auto lifecycle fixture must classify the holder as evaluator_workspace_publication_staging_root")

        old_store_ref = _write_publish_tree(
            repo_root / "artifacts" / "heavy_objects" / "publish" / "old-evaluator-auto-lifecycle-tree",
            label="evaluator-publish-tree-old",
        )
        old_id = str(heavy_object_identity(old_store_ref, object_kind="publish_tree").get("object_id") or "")
        _write_publish_tree(holder_ref, label="evaluator-publish-tree-old")
        ...
        submit_heavy_object_registration_request(
            ...
            reason="register old canonical evaluator publish-tree before replacement",
        ...
        submit_heavy_object_reference_request(
            ...
            reason="attach old evaluator publish-tree to the same holder path",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
        )
        ...
        if int(control_plane.get("pid") or 0) <= 0:
            return _fail("repo-global evaluator publish-tree auto lifecycle requires a live repo control-plane service")

        new_store_ref = _write_publish_tree(
            repo_root / "artifacts" / "heavy_objects" / "publish" / "new-evaluator-auto-lifecycle-tree",
            label="evaluator-publish-tree-new",
        )
        new_id = str(heavy_object_identity(new_store_ref, object_kind="publish_tree").get("object_id") or "")
        _write_publish_tree(holder_ref, label="evaluator-publish-tree-new")
        submit_heavy_object_registration_request(
            anchor_root,
            object_id=new_id,
            object_kind="publish_tree",
            ...
            reason="register replacement canonical evaluator publish-tree",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
            registration_kind="manual_store_seed",
        )
        replacement_reference = submit_heavy_object_reference_request(
            ...
            reference_holder_ref=str(holder.get("reference_holder_ref") or ""),
            reference_kind="manual_store_seed_reference",
            reason="attach replacement evaluator publish-tree to the same holder path",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
        )
        ...
        if not _wait_until(_auto_lifecycle_ready, timeout_s=40.0):
            return _fail("repo-global control-plane must auto-converge evaluator publish-tree replacement through supersession, GC eligibility, and reclamation")
        ...
        if len(supersession_events) != 1 or len(gc_eligibility_events) != 1 or len(reclaimed_events) != 1:
            return _fail("unchanged evaluator publish-tree replacement snapshot must converge through exactly one canonical lifecycle chain")
        if len(release_events) != 1:
            return _fail("unchanged evaluator publish-tree replacement snapshot must converge through exactly one canonical handoff release")
        ...
        if not replacement_reference_event_id:
            return _fail("evaluator publish-tree auto lifecycle test must locate the canonical replacement reference event")
        if str(release_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("evaluator publish-tree auto handoff release must preserve heavy_object_reference_replaced trigger kind")
        if str(release_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("evaluator publish-tree auto handoff release must link back to the replacing canonical reference event")
        ...
        if str(supersession_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("evaluator publish-tree auto supersession must preserve heavy_object_reference_replaced trigger kind")
        if str(supersession_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("evaluator publish-tree auto supersession must link back to the replacing canonical reference event")
        if str(gc_payload.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("evaluator publish-tree auto GC eligibility must preserve heavy_object_auto_supersession trigger kind")
        if str(reclaimed_payload.get("trigger_kind") or "") != "heavy_object_auto_gc_eligibility":
            return _fail("evaluator publish-tree auto reclamation must preserve heavy_object_auto_gc_eligibility trigger kind")
        ...
        if str(released_reference.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("evaluator publish-tree auto handoff release trace must preserve the replacement reference event id")
        ...
        if str(supersession_summary.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("evaluator publish-tree auto supersession trace must preserve the replacement reference event id")
        ...
        if str(gc_summary.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("evaluator publish-tree auto GC eligibility trace must preserve trigger_kind")
        ...
        if not bool(reclamation_effect.get("reclaimed")):
            return _fail("evaluator publish-tree auto reclamation trace must show reclaimed=true")
        ...
        if int(old_reference_inventory.get("active_reference_count") or 0) != 0:
            return _fail("evaluator publish-tree old object must lose active authoritative references after handoff release")
        if int(old_reference_inventory.get("released_reference_count") or 0) != 1:
            return _fail("evaluator publish-tree old object must expose one released authoritative reference after handoff release")
        ...
        if int(new_reference_inventory.get("active_reference_count") or 0) != 1:
            return _fail("evaluator publish-tree replacement identity must retain one active authoritative reference after handoff release")
        if list(new_reference_inventory.get("active_reference_refs") or []) != [str(holder_ref)]:
            return _fail("evaluator publish-tree replacement identity must preserve the evaluator holder reference after handoff release")

        new_view = query_heavy_object_retention_view(anchor_root, object_id=new_id)
        if str(new_view.get("retention_state") or "") != "REFERENCED":
            return _fail("evaluator publish-tree replacement identity must remain REFERENCED after auto handoff convergence")
        ...
        if str(retention_view.get("retention_state") or "") != "RECLAIMED":
            return _fail("evaluator publish-tree retention view must converge old object to RECLAIMED")
        ...
        if len(
            ...
            return _fail("unchanged evaluator publish-tree replacement snapshot must not emit duplicate auto handoff release facts")

    print(
        "[loop-system-repo-global-evaluator-publish-tree-auto-lifecycle][OK] "
        "repo-global evaluator publish-tree replacement deterministically auto-converges through canonical handoff release and lifecycle events"
    )
    return 0
