#!/usr/bin/env python3
"""Validate repo-global auto lifecycle convergence for runtime-heavy-tree replacements."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-runtime-heavy-tree-auto-lifecycle][FAIL] {msg}", file=sys.stderr)
    return 2
...
def _persist_anchor_state(state_root: Path) -> None:
    ...
    kernel_state = KernelState(
        task_id="milestone5-runtime-heavy-tree-auto-lifecycle",
        root_goal="validate repo-global runtime-heavy-tree auto lifecycle convergence",
        root_node_id="root-kernel",
    )
    root_node = NodeSpec(
        node_id="root-kernel",
        node_kind="kernel",
        goal_slice="supervise repo-global runtime-heavy-tree auto lifecycle validation",
        parent_node_id=None,
        generation=0,
        round_id="R0",
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_repo_global_runtime_heavy_tree_auto_lifecycle_") as repo_root:
        anchor_root = repo_root / ".loop"
        _persist_anchor_state(anchor_root)

        holder_root = repo_root / "workspace" / "runtime-heavy-tree-auto-lifecycle" / ".tmp_primary_artifact"
        holder_ref = _write_runtime_heavy_tree(holder_root, label="runtime-heavy-tree-live")
        ...
        if str(holder.get("reference_holder_kind") or "") != "runtime_workspace_live_artifact_root":
            return _fail("runtime-heavy-tree auto lifecycle fixture must classify the holder as runtime_workspace_live_artifact_root")

        old_store_ref = _write_runtime_heavy_tree(
            repo_root / "artifacts" / "heavy_objects" / "runtime-heavy" / "old-auto-lifecycle-tree",
            label="runtime-heavy-tree-old",
        )
        old_id = str(heavy_object_identity(old_store_ref, object_kind="runtime_heavy_tree").get("object_id") or "")
        _write_runtime_heavy_tree(holder_root, label="runtime-heavy-tree-old")
        ...
        submit_heavy_object_registration_request(
            ...
            reason="register old canonical runtime-heavy-tree before replacement",
            runtime_name="runtime-heavy-runtime",
            repo_root=repo_root,
        ...
        submit_heavy_object_reference_request(
            ...
            reason="attach old runtime-heavy-tree to the live holder path",
            runtime_name="runtime-heavy-runtime",
            repo_root=repo_root,
        )
        ...
        if int(control_plane.get("pid") or 0) <= 0:
            return _fail("repo-global runtime-heavy-tree auto lifecycle requires a live repo control-plane service")

        new_store_ref = _write_runtime_heavy_tree(
            repo_root / "artifacts" / "heavy_objects" / "runtime-heavy" / "new-auto-lifecycle-tree",
            label="runtime-heavy-tree-new",
        )
        new_id = str(heavy_object_identity(new_store_ref, object_kind="runtime_heavy_tree").get("object_id") or "")
        _write_runtime_heavy_tree(holder_root, label="runtime-heavy-tree-new")
        submit_heavy_object_registration_request(
            anchor_root,
            object_id=new_id,
            object_kind="runtime_heavy_tree",
            ...
            reason="register replacement canonical runtime-heavy-tree",
            runtime_name="runtime-heavy-runtime",
            repo_root=repo_root,
            registration_kind="manual_store_seed",
        )
        replacement_reference = submit_heavy_object_reference_request(
            ...
            reference_holder_ref=str(holder.get("reference_holder_ref") or ""),
            reference_kind="manual_store_seed_reference",
            reason="attach replacement runtime-heavy-tree to the same live holder path",
            runtime_name="runtime-heavy-runtime",
            repo_root=repo_root,
        )
        ...
        if not _wait_until(_auto_lifecycle_ready, timeout_s=40.0):
            return _fail(
                "repo-global control-plane must auto-converge runtime-heavy-tree replacement through supersession, "
        ...
        if (
            ...
            return _fail("unchanged runtime-heavy-tree replacement snapshot must converge through exactly one canonical release plus one lifecycle chain")
        ...
        if not replacement_reference_event_id:
            return _fail("runtime-heavy-tree auto lifecycle test must locate the canonical replacement reference event")
        if str(release_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("runtime-heavy-tree auto handoff release must preserve heavy_object_reference_replaced trigger kind")
        if str(release_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("runtime-heavy-tree auto handoff release must link back to the replacing canonical reference event")
        if str(supersession_payload.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("runtime-heavy-tree auto supersession must preserve heavy_object_reference_replaced trigger kind")
        if str(supersession_payload.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("runtime-heavy-tree auto supersession must link back to the replacing canonical reference event")
        if str(gc_payload.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("runtime-heavy-tree auto GC eligibility must preserve heavy_object_auto_supersession trigger kind")
        if str(gc_payload.get("trigger_ref") or "") != str(supersession_event.get("event_id") or ""):
            return _fail("runtime-heavy-tree auto GC eligibility must link back to the canonical supersession event")
        if str(reclaimed_payload.get("trigger_kind") or "") != "heavy_object_auto_gc_eligibility":
            return _fail("runtime-heavy-tree auto reclamation must preserve heavy_object_auto_gc_eligibility trigger kind")
        if str(reclaimed_payload.get("trigger_ref") or "") != str(gc_eligibility_event.get("event_id") or ""):
            return _fail("runtime-heavy-tree auto reclamation must link back to the canonical GC-eligibility event")
        ...
        if str(released_reference.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("runtime-heavy-tree auto handoff release trace must preserve trigger_kind")
        if str(released_reference.get("trigger_ref") or "") != replacement_reference_event_id:
            return _fail("runtime-heavy-tree auto handoff release trace must preserve trigger_ref")
        ...
        if str(supersession_summary.get("trigger_kind") or "") != "heavy_object_reference_replaced":
            return _fail("runtime-heavy-tree auto supersession trace must preserve trigger_kind")
        ...
        if str(eligible_object.get("trigger_kind") or "") != "heavy_object_auto_supersession":
            return _fail("runtime-heavy-tree auto GC trace must preserve trigger_kind")
        ...
        if str(reclamation_effect.get("trigger_kind") or "") != "heavy_object_auto_gc_eligibility":
            return _fail("runtime-heavy-tree auto reclamation trace must preserve trigger_kind")
        ...
        if int(old_reference_inventory.get("active_reference_count") or 0) != 0:
            return _fail("replaced runtime-heavy-tree object must no longer have active authoritative references after handoff release")
        if int(old_reference_inventory.get("released_reference_count") or 0) != 1:
            return _fail("replaced runtime-heavy-tree object must expose one released authoritative reference after handoff release")
        ...
        if int(new_reference_inventory.get("active_reference_count") or 0) != 1:
            return _fail("replacement runtime-heavy-tree identity must retain one active authoritative reference after handoff release")
        if list(new_reference_inventory.get("active_reference_refs") or []) != [str(holder_ref)]:
            return _fail("replacement runtime-heavy-tree identity must preserve the live holder reference after handoff release")
        ...
        if str(old_view.get("retention_state") or "") != "RECLAIMED":
            return _fail("runtime-heavy-tree auto lifecycle must drive the replaced object to retention_state=RECLAIMED")
        if old_store_ref.exists():
            return _fail("runtime-heavy-tree auto lifecycle must reclaim the superseded canonical directory ref")
        new_view = query_heavy_object_retention_view(anchor_root, object_id=new_id)
        if str(new_view.get("retention_state") or "") != "REFERENCED":
            return _fail("replacement runtime-heavy-tree identity must remain REFERENCED after auto handoff convergence")
        ...
        if len(
            ...
            return _fail("unchanged runtime-heavy-tree replacement snapshot must not emit duplicate auto handoff release facts")
        if len(
            [
                dict(event)
                for event in _committed_events_by_type(anchor_root, "heavy_object_superseded")
            ...
            return _fail("unchanged runtime-heavy-tree replacement snapshot must not emit duplicate auto supersession facts")

    print(
        "[loop-system-repo-global-runtime-heavy-tree-auto-lifecycle][OK] "
        "repo-global control-plane deterministically auto-converges runtime-heavy-tree replacement through canonical handoff release, supersession, GC eligibility, and reclamation"
    )
    return 0
