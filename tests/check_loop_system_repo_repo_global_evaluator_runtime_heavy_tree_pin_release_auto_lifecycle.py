#!/usr/bin/env python3
"""Validate repo-global evaluator runtime-heavy-tree auto lifecycle unblocks after authoritative pin release."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-evaluator-runtime-heavy-tree-pin-release-auto-lifecycle][FAIL] {msg}", file=sys.stderr)
    return 2
...
def _persist_anchor_state(state_root: Path) -> None:
    ...
    kernel_state = KernelState(
        task_id="milestone5-repo-global-evaluator-runtime-heavy-tree-pin-release-auto-lifecycle",
        root_goal="validate repo-global evaluator runtime-heavy-tree auto lifecycle unblock after pin release",
        root_node_id="root-kernel",
    )
    root_node = NodeSpec(
        node_id="root-kernel",
        node_kind="kernel",
        goal_slice="supervise repo-global evaluator runtime-heavy-tree pin-release auto lifecycle validation",
        parent_node_id=None,
        generation=0,
        round_id="R0",
...
def main() -> int:
    ...
    with temporary_repo_root(prefix="loop_system_repo_global_evaluator_runtime_heavy_tree_pin_release_auto_lifecycle_") as repo_root:
        ...
        evaluator_workspace_root = (
            repo_root
            / ".loop"
            / "evaluator-runtime-heavy-tree-pin-release-auto-lifecycle"
            / "artifacts"
            / "evaluator_runs"
            / "lane_a"
            / ".loop"
        ...
        holder_root = evaluator_workspace_root / "deliverables" / "primary_artifact"
        ...
        if str(holder.get("reference_holder_kind") or "") != "evaluator_workspace_artifact_root":
            return _fail("evaluator runtime-heavy-tree pin-release auto lifecycle fixture must classify the holder as evaluator_workspace_artifact_root")

        old_store_ref = _write_runtime_heavy_tree(
            repo_root / "artifacts" / "heavy_objects" / "runtime-heavy" / "old-evaluator-pin-release-auto-lifecycle-tree",
        ...
        submit_heavy_object_pin_request(
            ...
            reason="keep the old evaluator runtime-heavy-tree retained while replacement is still pinned by policy",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
        )
        ...
        if int(control_plane.get("pid") or 0) <= 0:
            return _fail("repo-global evaluator runtime-heavy-tree pin-release auto lifecycle requires a live repo control-plane service")

        new_store_ref = _write_runtime_heavy_tree(
            repo_root / "artifacts" / "heavy_objects" / "runtime-heavy" / "new-evaluator-pin-release-auto-lifecycle-tree",
        ...
        if not _wait_until(_superseded_but_not_gc_or_reclaimed, timeout_s=8.0):
            return _fail("while the old evaluator runtime-heavy-tree remains pinned, repo-global auto lifecycle must stop after canonical supersession")
        ...
        if str(pinned_view.get("retention_state") or "") != "PINNED":
            return _fail("old superseded evaluator runtime-heavy-tree must remain PINNED while the authoritative pin is active")
        if not old_store_ref.exists():
            return _fail("old canonical evaluator runtime-heavy-tree ref must still exist while the authoritative pin blocks reclamation")
        ...
        released = submit_heavy_object_pin_release_request(
            ...
            pin_holder_id="publish:artifact-bundle",
            pin_kind="publish_retention_pin",
            reason="release the policy pin so repo-global lifecycle may reclaim the superseded evaluator runtime-heavy-tree",
            runtime_name="evaluator-runtime",
            repo_root=repo_root,
        )
        ...
        if not _wait_until(_gc_and_reclaimed_after_release, timeout_s=8.0):
            return _fail("after authoritative pin release, repo-global evaluator runtime-heavy-tree lifecycle must resume GC eligibility and reclamation")
        ...
        if str(released_pin.get("pin_holder_id") or "") != "publish:artifact-bundle":
            return _fail("evaluator runtime-heavy-tree pin-release trace must preserve the released pin holder id")
    ...
    print(
        "[loop-system-repo-global-evaluator-runtime-heavy-tree-pin-release-auto-lifecycle][OK] "
        "repo-global control-plane keeps pinned superseded evaluator runtime-heavy-trees retained and then resumes authoritative lifecycle after pin release"
    )
    return 0
