#!/usr/bin/env python3
"""Validate full repo-global stack restoration from surviving housekeeping only."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-housekeeping-chain-restore][FAIL] {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loop_system_repo_global_housekeeping_chain_restore_") as td:
        ...
        try:
            ...
            if min(guard_pid, reactor_pid, control_plane_pid, supervisor_pid, housekeeping_pid) <= 0:
                return _fail("setup must expose the full live repo-global service stack")
            ...
            if not _wait_until(
                ...
                return _fail("test setup must be able to terminate the old upper repo-global service pids")
            ...
            if not _process_alive(
                ...
                return _fail("housekeeping must remain alive to drive chain restoration")

            if not _wait_until(
                lambda: (
                    (housekeeping := live_housekeeping_reap_controller_runtime(repo_root=repo_root)) is not None
                ...
                return _fail(
                    "surviving housekeeping must restore repo control-plane, which must restore the upper reactor layer and then reconverge the full repo-global stack"
                )
            ...
            control_plane_required_events = [
                ...
                and str(dict(event.get("payload") or {}).get("trigger_kind") or "") == "housekeeping_watchdog"
            ]
            if not control_plane_required_events:
                return _fail("housekeeping chain restore must publish canonical repo_control_plane_required")

            residency_required_events = [
                event
            ...
            if not residency_required_events:
                return _fail(
                    "housekeeping chain restore must converge through repo-control-plane-published repo_reactor_residency_required facts"
                )
        finally:
            _cleanup_repo_services(repo_root)

    print("[loop-system-repo-global-housekeeping-chain-restore] OK")
    return 0
