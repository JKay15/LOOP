#!/usr/bin/env python3
"""Validate repo-global reactor restoration of repo services."""
...
def _fail(msg: str) -> int:
    print(f"[loop-system-repo-global-reactor-restore][FAIL] {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="loop_system_repo_global_reactor_restore_") as td:
        ...
        try:
            ...
            if reactor_pid <= 0 or control_plane_pid <= 0 or supervisor_pid <= 0 or housekeeping_pid <= 0:
                return _fail("setup must expose live repo reactor, repo control-plane, host supervisor, and housekeeping pids")

            if not _process_alive(reactor_pid, "loop_product.runtime.control_plane --run-repo-reactor", repo_root=repo_root):
                return _fail("repo reactor bootstrap must start a live repo reactor process")
            ...
            if not _wait_until(
                lambda: (
                    not _process_alive(control_plane_pid, "loop_product.runtime.control_plane", repo_root=repo_root)
                ...
                return _fail("test setup must be able to terminate old repo service pids")

            if not _process_alive(reactor_pid, "loop_product.runtime.control_plane --run-repo-reactor", repo_root=repo_root):
                return _fail("repo reactor must remain alive after subordinate repo services die")

            if not _wait_until(
                lambda: (
                    (reactor := live_repo_reactor_runtime(repo_root=repo_root)) is not None
                ...
                return _fail("surviving repo reactor must restore repo control-plane, host supervisor, and housekeeping")
            ...
            if not reactor_required_events:
                return _fail("repo reactor bootstrap must publish canonical repo_reactor_required")

            control_plane_required_events = [
                event
            ...
            if not control_plane_required_events:
                return _fail("repo reactor restore must publish canonical repo_control_plane_required from reactor watchdog")
        finally:
            _cleanup_repo_services(repo_root)

    print("[loop-system-repo-global-reactor-restore] OK")
    return 0
