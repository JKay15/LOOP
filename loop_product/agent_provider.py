...
def _load_profile(path: Path, *, repo_root: Path) -> Dict[str, object]:
    try:
        ...
        raise RuntimeError(
            f"agent profile validation needs jsonschema; install jsonschema before loading agent profiles from {path}"
        ) from exc
...
def _build_default_command(*, provider_id: str, prompt_transport: str, prompt_arg: str) -> str:
    ...
    prompt_env = '"$LOOP_PRODUCT_EVAL_PROMPT"'
...
def _resolve_from_profile(
    *,
    ...
    return ResolvedAgentInvocation(
        ...
        profile_selector=str(profile_path),
...
def resolve_agent_invocation(
    *,
    ...
    if profile_raw:
        p = Path(profile_raw).expanduser()
        resolved_path = p if p.is_absolute() else (Path.cwd() / p)
        resolved_path = resolved_path.resolve()
        if resolved_path.exists():
            return _resolve_from_profile(repo_root=repo_root, profile_path=resolved_path, override_provider=provider_raw)
        if provider_raw:
            return _resolve_from_provider(
                provider_raw,
                profile_selector=profile_raw,
                source="cli.agent_profile_selector",
            )
        raise FileNotFoundError(f"--agent-profile not found: {resolved_path}")
