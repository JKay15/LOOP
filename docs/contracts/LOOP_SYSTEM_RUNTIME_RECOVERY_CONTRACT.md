...
- that committed child runtime status helper must also scrub runtime-owned heavy trees from live child artifact roots such as `deliverables/primary_artifact` or `.tmp_primary_artifact` before returning ordinary ACTIVE status, so live benchmark continuation does not rely on evaluator-time cleanup alone
