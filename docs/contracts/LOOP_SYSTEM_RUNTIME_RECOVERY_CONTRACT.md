...
- accepted same-node orphaned-active recovery must also refresh the exact node-local frozen bundle under current code before relaunching: `FROZEN_HANDOFF.*`, `CHILD_PROMPT.md`, evaluator bundle files, and publication/evaluator runners must be rematerialized through the exact bootstrap path rather than reusing stale bundle bytes from before the maintainer's latest runtime/evaluator hardening wave
