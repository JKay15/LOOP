...
- that committed child runtime status helper must distinguish the live build root from the publish root: `.tmp_primary_artifact` may contain local build state, but direct mutation of `deliverables/primary_artifact` without a matching runtime-owned publication receipt is a publication violation
