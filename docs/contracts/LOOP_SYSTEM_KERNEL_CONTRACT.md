...
- public trusted authoritative state sync surface: `loop_product.runtime.synchronize_authoritative_state(...)`
- public trusted authoritative terminal-result backfill surface: `loop_product.runtime.backfill_authoritative_terminal_results(...)`
...
- `accepted_envelopes.json` must be idempotent by `envelope_id`; replay of an already accepted envelope may be audited, but must not append a second accepted fact or re-trigger kernel side effects
...
- deferred split is a separate accepted mode: accepted future children materialize as `PLANNED`, and kernel preserves their dependency/activation metadata for later activation
- a deferred split source may later write an authoritative split-continuation result (`SPLIT_ACCEPTED_CONTINUE_IMPLEMENTATION`); kernel-owned authority sync must normalize that source into a blocked continuation segment instead of leaving it falsely `ACTIVE`
...
- deferred activation conditions may include split-release requirements such as `after:<source>:split_accepted_and_<slice>_released`; kernel/runtime must treat those requirements as supported only when the source has durably published the released slice under an authoritative split-continuation result
...
- when a source authoritative result records `SPLIT_ACCEPTED_CONTINUE_IMPLEMENTATION`, kernel-owned continuation must treat it as a durable split-release checkpoint rather than as an orphaned ACTIVE runtime-loss case
