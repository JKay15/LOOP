...
Every evaluator AI role also gets one dedicated role-specific working directory. The evaluator calling code must select that role by `cwd` / `-C`. It must also materialize a runtime-owned `CODEX_HOME` for that role so Codex CLI uses isolated mutable state (SQLite/session/log files) instead of the host default `~/.codex`, while still sharing the host auth/config surfaces needed for the current user/account. Per-run artifacts live under `run_root/.loop/<role>/runs/`, role guidance stays on committed prompts/runtime metadata instead of transient runtime `AGENTS.md` / `AGENTS.override.md` files, nested role launches must scrub parent `CODEX_*` session/transport env vars such as `CODEX_THREAD_ID` and `CODEX_INTERNAL_ORIGINATOR_OVERRIDE`, and if the nested role command is a repo-shipped Python module such as `python -m loop_product...`, the launch must still preserve repo importability from that dedicated `cwd`, for example by prepending the product repo root to `PYTHONPATH`.
...
If `implementation_package_ref` points at a workspace-local directory artifact, the evaluator-node adapter treats that directory as the exact shipped deliverable, not as a scratch build root. Evaluator launch must require a matching runtime-owned publication receipt for that publish root before any role lane starts.
That publication receipt must prove both that the shipped directory is free of runtime-owned heavy trees such as `.lake`, `.git`, `.venv`, `.uv-cache`, `build`, and `_lake_build`, and that the current live artifact root has been republished since its last material change. If the publish root drifted, was never published, or still contains runtime-owned trees, evaluator launch must fail closed instead of silently rescuing a dirty runtime snapshot.
If the evaluator submission comes from a split child or other narrowed slice that declares `required_output_paths`, evaluator launch must also fail closed until every declared required output exists in the shipped artifact. A child-authored blocked or terminal slice status does not excuse unrelated missing required outputs; the runtime must carry those required outputs from split review into durable node/handoff/submission state and the evaluator must enforce them before any checker/reviewer lanes start.
For whole-paper final integration, evaluator launch is also dependency-gated: even with a terminal `WHOLE_PAPER_STATUS.json`, the evaluator-node adapter must fail closed until every declared dependency node has reached a terminal-ready authoritative outcome.
