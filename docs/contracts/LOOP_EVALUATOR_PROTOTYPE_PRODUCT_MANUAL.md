...
That evaluator-side canonicalization is only the last fail-closed guard. The wider runtime should also scrub those same heavy trees from live child deliverables and staging artifacts during ordinary authority sync, instead of letting parallel child workspaces accumulate `.lake` or other runtime-owned trees until evaluator time.
