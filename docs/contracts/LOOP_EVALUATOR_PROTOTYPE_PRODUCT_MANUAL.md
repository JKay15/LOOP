...
When those staged role workspaces are copied from the source workspace, runtime-owned/generated directories such as `.loop/`, `.loop_runtime/`, `workspace/`, and `.uv-cache/` should stay excluded by default so delegated evaluator roles do not inherit stale sibling outputs or old runtime ledgers.
