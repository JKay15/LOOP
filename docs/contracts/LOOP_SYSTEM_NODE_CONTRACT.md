...
- committed child runtime status polling should act as a live hygiene barrier for ACTIVE child workspaces: runtime-owned heavy trees that appear under `deliverables/primary_artifact` or `.tmp_primary_artifact` must be scrubbed during ordinary status polling, not only during later authority sync or evaluator preflight
