...
- if a bootstrap request or frozen handoff explicitly carries the committed shared-cache helper in `context_refs`, runtime should treat that helper ref as a machine-auditable request to preseed the external live-root `.lake` mount before launch instead of leaving helper invocation as advisory prompt text
...
- startup-contract hardening applies to split children as well as source nodes: whole-paper source nodes and whole-paper slice nodes may use different deterministic startup bundles, but neither class may launch with an empty startup contract and then rely on generic patience alone
- whole-paper source nodes should default to the deterministic extraction/partition/split control bundle, while whole-paper slice nodes should default to a deterministic slice startup bundle such as `README.md`, `TRACEABILITY.md`, `SLICE_STATUS.json`, and `analysis/SLICE_BOUNDARY.json`
- whole-paper slice nodes should also default to a machine-auditable post-startup decision checkpoint such as `analysis/SLICE_EXECUTION_DECISION.json` (or an explicit split submission artifact), so supervision does not have to guess whether a long planning interval reflects valuable slice reasoning or simple drift
- the purpose of those node-class-specific startup bundles is to delay supervision judgment until the machine-auditable startup surface exists, not to excuse indefinite post-startup planning drift; after startup, later checkpoint or artifact-progress rules still apply
...
- once supervision has durably written a settled `ChildSupervisionResult.json`, committed runtime-status/supervision surfaces must reconcile that stop point back into durable node state even when no authoritative terminal `result.json` exists yet; a truthfully settled node must not remain falsely `ACTIVE/UNOBSERVED`
