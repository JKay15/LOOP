...
- the live build root and the shipped publish root are distinct surfaces: `.tmp_primary_artifact` (or the exact live artifact root named in handoff) is the only valid scratch/build root, while `deliverables/primary_artifact` is publish-only
