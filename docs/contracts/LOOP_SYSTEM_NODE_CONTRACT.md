...
- final workspace-mirror artifacts should exclude runtime-owned heavy trees such as `.lake`, `.git`, `.venv`, `.uv-cache`, `build`, and `_lake_build`; when artifact-local build support rematerializes those trees locally, the implementer should prune them again before evaluator or final report claims the shipped artifact is final
